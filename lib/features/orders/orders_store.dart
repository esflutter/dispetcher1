...
class MyOrdersStore {
  ...
  static Future<void> _cancelOrderWithRollback({
    ...
    try {
      ...
      // Гонка: пока мы отменяли, заказ уже перешёл в терминальный статус в БД
      // (сервер отвечает «Заказ не найден или уже в терминальном статусе»).
      // Откатывать перенос в архив тут НЕЛЬЗЯ — в БД заказ уже неактивен, и
      // возврат его в «Новые»/«Принятые» показал бы отменённый заказ живым.
      // Просто оповещаем UI, локальное состояние оставляем как есть.
      if (e.toString().contains('Заказ не найден или уже в терминальном статусе')) {
        onError?.call(e);
        return;
      }
