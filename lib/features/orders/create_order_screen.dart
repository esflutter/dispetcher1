...
class DailyOrderLimit {
  ...
  static void resetToday() {
    // Метку дня ставим в той же системе, что и остальные методы счётчика
    // (начало московских суток в UTC). Раньше тут была ЛОКАЛЬНАЯ полночь —
    // она никогда не равна UTC-границе, поэтому первая же проверка лимита
    // сбрасывала счётчик повторно. Вреда не было (он и так 0), но формат
    // метки расходился — приводим к единому виду.
    _date = _moscowDayStartUtc();
    _count = 0;
  }
...
class _CreateOrderScreenState extends State<CreateOrderScreen> {
  ...
  void _applyAiDraft(
    ...
    mapIds();
    // Если справочники ещё не догрузились — дождёмся реальной загрузки и
    // повторим маппинг. Раньше тут была фиксированная пауза 100 мс: на
    ...
    if (mcCached == null) {
      CatalogService.instance.warmup().then((_) {
        ...
        // Прогрев справочника не удался (нет сети) — техника/категория из
        // черновика подставятся при обычной загрузке экрана. Молча гасим,
        // чтобы исключение не ушло необработанным в общий обработчик.
      });
    }
