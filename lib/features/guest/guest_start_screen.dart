...
class GuestStartScreen extends StatelessWidget {
  ...
  Widget _hero() => Container(
    ...
    child: Column(
      ...
      children: <Widget>[
        ...
        SizedBox(height: 8.h),
        Text(
          'Создайте заказ — исполнители сами предложат помощь.',
    ...
    child: Column(
      ...
      children: <Widget>[
        Container(
          ...
          child: Icon(Icons.add_task_rounded, color: Colors.white, size: 25.r),
        ),
        SizedBox(height: 14.h),
        Text('Разместите первый заказ', style: AppTextStyles.titleL),
        SizedBox(height: 6.h),
        Text(
          'Регистрация займёт около минуты. После неё сразу откроется форма заказа.',
        ...
        PrimaryButton(
          label: 'Создать заказ',
          onPressed: () =>
              startGuestAuth(context, intent: GuestAuthIntent.createOrder),
        ),
      ],
    ),
  );
  ...
  Widget _catalogAction(BuildContext context) => InkWell(
    ...
    onTap: () => showGuestAuthPrompt(
      context,
      message:
          'Войдите, чтобы смотреть исполнителей, их контакты и предложения.',
        ...
        children: <Widget>[
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: <Widget>[
                Text('Каталог исполнителей', style: AppTextStyles.titleS),
                SizedBox(height: 3.h),
                Text(
