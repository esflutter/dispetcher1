  ...
  if (Env.hasSupabaseConfig) {
    ...
    Supabase.instance.client.auth.onAuthStateChange
      ...
      if (event.event == AuthChangeEvent.signedOut) {
        ...
        // Вход/регистрация (в т.ч. из гостевого режима). Гость теперь может
        // пользоваться ассистентом — его переписка живёт в статической ленте
        // чата и в гостевой серверной сессии (по device_id). При входе чистим
        // их, чтобы они НЕ «прилипли» к только что вошедшему аккаунту: новый
        // юзер должен начинать ассистента с чистого приветствия. resetHistory
        // дёргает и AiClient.resetSessions() (память + сохранённые id), после
        // чего своя история подтянется из БД уже под этим пользователем.
