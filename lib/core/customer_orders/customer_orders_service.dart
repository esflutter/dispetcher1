...
class CustomerOrdersService {
  ...
  Future<({String matchId, String serviceId})> proposeOrderToExecutor({
    ...
    // одного без ручного отказа от другого. На уровне БД это гарантирует
    // уникальный частичный индекс order_matches_one_awaiting_executor на
    // (order_id) WHERE status = 'awaiting_executor': при гонке второй INSERT
    // упадёт с 23505 (обрабатывается в catch ниже). Этот SELECT — быстрая
    // проверка для типичного случая и понятная ошибка пользователю.
