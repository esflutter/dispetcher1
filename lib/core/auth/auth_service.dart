...
String authErrorToRu(Object error) {
  ...
  // Бан номера: триггер регистрации нового пользователя падает
  // («Database error saving new user»), когда телефон в чёрном списке —
  // так сервер закрывает повторную регистрацию после блокировки прошлого
  // аккаунта. Общий фолбэк тут вводит в заблуждение (человек жмёт «ещё раз»).
  if (lower.contains('database error') && lower.contains('saving new user')) {
    return 'Регистрация по этому номеру недоступна — действует блокировка '
        'предыдущего аккаунта. Если это ошибка, напишите в поддержку.';
  }
